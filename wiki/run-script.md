...
needed. Navigating mid-script would tear down the evaluation context, so the page
must already be on `deepwiki.com`; navigate first with
`chrome-devtools navigate https://deepwiki.com`.
...
### CLI Execution
```bash
# Navigate to the target site first, then run the script.
chrome-devtools --target warm-squid navigate https://deepwiki.com
